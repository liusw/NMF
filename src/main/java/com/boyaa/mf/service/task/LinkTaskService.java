...
public class LinkTaskService {
	...
	public ResultState param2TaskJson(HttpServletRequest request){
		...
		try{
			JSONObject paramJson = JSONUtil.parseObject(taskInfo);
			JSONArray paramJsonArray = (JSONArray) paramJson.get("processList");
			
			String outputColumn = (String) JsonUtils.getValue(paramJson, "outputColumn", "");
			int outputType = (Integer) JsonUtils.getValue(paramJson, "outputType",0);
			String outputLink = (String) JsonUtils.getValue(paramJson, "outputLink", "");
			
			String receiveEmail = (String) JsonUtils.getValue(paramJson, "email", "");
			if(StringUtils.isBlank(receiveEmail)){
				...
				receiveEmail = userInfo.getEmail();
			}
			...
			jsonObject.put("outputType",outputType);
			jsonObject.put("outputColumn",outputColumn);
			jsonObject.put("outputLink",outputLink);
	...
	public JSONObject hiveProcessService(JSONObject jsonParam) throws JSONException, ParseException{
		...
		String dependOn = (String) JsonUtils.getValue(jsonParam, "dependOn", "");
		String linkParam = (String) JsonUtils.getValue(jsonParam, "linkParam", "");
		String distinctType = (String) JsonUtils.getValue(jsonParam,"distinctType", "");
		//String format = (String) JsonUtils.getValue(jsonParam,"format", "");
		String realTimeData = (String) JsonUtils.getValue(jsonParam,"realTimeData", "");
		
		String orderSql=null;
		if(StringUtils.isNotBlank(tableName) && tableName.trim().equals("user_order2")){
			//获取user_order2的所有字段
			StringBuffer sb = new StringBuffer();
			try {
				JSONArray columnInfo = hiveMetaService.getColumnInfo(tableName);
				JSONObject jsonObject = null;
				for(int i=0;i<columnInfo.size();i++){
					jsonObject = columnInfo.getJSONObject(i);
					
					sb.append(i==0?"":",").append(jsonObject.getString("column_name"));
				}
			...
			if(sb.toString().length()>0){
				String orderColumns = HiveUtils.columsSpecialSymbols(sb.toString());
				orderSql = "select "+ orderColumns + " from "+
									"(select "+orderColumns+",row_number() over (),rank() over(partition by `_plat`,`_uid`,`pid` order by row_number() over (),`_tm` desc,utime desc) as rank  from "+
									tableName+" where plat="+plat+ HiveUtils.getWhereBetweenTm(startTime, endTime, tableName)+") subquery " +
							"where subquery.rank=1 and subquery.pstatus=2";
			}
		}
...
//		}
		//处理特殊符号:_uid-->`_uid`
		String originalColumns = columns;//没有处理过,例如`号,如果有`也不做处理
...
//		}
		...
		String queryTableColumn = (String) columnMap.get("queryTableColumn");
		...
		if(columnMap.containsKey("common")){
			...
			originalColumns +=(","+paramMap.get("asName"));
			outputTitle +=(","+paramMap.get("title"));
		}
		if(columnMap.containsKey("format")){
			Map<String,String> paramMap = (Map<String, String>) columnMap.get("format");
			queryColumn +=(","+paramMap.get("column"));
			outputColumn +=(","+paramMap.get("asName"));
			originalColumns +=(","+paramMap.get("asName"));
			outputTitle +=(","+paramMap.get("title"));
	   ...
	   finalQueryColumn= queryColumn;
	   sb.append("select ").append("#finalQueryColumn#").append(" from ")
	     .append(StringUtils.isNotBlank(orderSql)?"("+orderSql+") x ":tableName).append(" where 1=1 ");
	   if(partitionjson!=null && partitionjson.containsKey("_plat") && !StringUtils.equals(realTimeData, "1")){
			String partitionPlat = partitionjson.getString("_plat");
			if(partitionPlat.indexOf("rename:")!=-1){
				partitionPlat = partitionPlat.substring(partitionPlat.indexOf("rename:")+7);
			}else{
				partitionPlat = "plat";
			}
		   sb.append(" and "+partitionPlat+"=").append(hplat);
	   }else{
		   sb.append(" and `_plat`=").append(hplat);
	   	}
	   sb.append(HiveUtils.getWhereBetweenTm(startTime, endTime, tableName,!StringUtils.equals(realTimeData,"1")));
	   ...
	   if(StringUtils.isNotBlank(dependOn)){
			sb.append(") b on(").append(HiveUtils.onCondition("b","a",linkParam)).append(") where 1=1 and (");
			String[] cArr = columns.split(",");
			for(int i=0;i<cArr.length;i++){
				sb.append(i==0?"":" or ").append(" b."+cArr[i]+" is not null ");
			}
			sb.append(")");
		}
	   
	   //on的条件如果没有在查询的列中,要把查询的列添加到查询语句中
	   finalQueryColumn = addOnColumn(linkParam,finalQueryColumn);
	  ...
	  if(StringUtils.equals(distinctType, "3") || StringUtils.equals(distinctType, "4")){//获取最后一条记录
		  ...
		  finalQueryColumn = queryTableColumn;
	  }else if(StringUtils.equals(distinctType, "1") || StringUtils.equals(distinctType, "2") || StringUtils.equals(distinctType, "5")){
		  if(StringUtils.equals(distinctType, "1")){
			  queryColumn = "`_uid`";
			  outputColumn = "`_uid`";
			  originalColumns = "_uid";
			  outputTitle = "用户ID";
		  }else if(StringUtils.equals(distinctType, "2")){
			  queryColumn = "`_uid`,"+HiveUtils.getTmDate(tableName,!StringUtils.equals(realTimeData, "1"));
			  outputColumn = "`_uid`,tm";
			  originalColumns = "_uid,tm";
			  outputTitle = "用户ID,日期";
		  }else if(StringUtils.equals(distinctType, "5")){
			  queryColumn = HiveUtils.getTmDate(tableName,!StringUtils.equals(realTimeData, "1"));
			  outputColumn = "tm";
			  originalColumns = "tm";
			  outputTitle = "日期";
		  }
		  ...
		  if(StringUtils.equals(distinctType, "1")){
			  ...
			  uniqueSb.append("select "+queryColumn+" from( ").append(sb.toString()).append(" )t group by `_uid`,").append(HiveUtils.getTmDate(tableName,!StringUtils.equals(realTimeData, "1")));
		  }else if(StringUtils.equals(distinctType, "5")){
			  uniqueSb.append("select "+queryColumn+" from( ").append(sb.toString()).append(" )t group by ").append(HiveUtils.getTmDate(tableName,!StringUtils.equals(realTimeData, "1")));
		  }
	  }else{
		  String filterDuplicateData = (String) JsonUtils.getValue(jsonParam, "filterDuplicateData", "0");
		  if(filterDuplicateData.equals("1")){
			  uniqueSb.append("select * from (").append(sb.toString()).append(") t group by "+outputColumn);
		  }else{
			  uniqueSb.append(sb.toString());
		  }
	   }
		
	  jsonObject.put("columnName",originalColumns);
	  jsonObject.put("title",outputTitle);
	  jsonObject.put("operation",uniqueSb.toString().replace("#finalQueryColumn#", finalQueryColumn));
	 ...
	 * @param string
	 * @param linkParam
	 * @param finalQueryColumn
	 * @return
	 */
	private String addOnColumn(String linkParam,String finalQueryColumn) {
		if (StringUtils.isBlank(linkParam)) return finalQueryColumn;
		
		String[] fArray = finalQueryColumn.split(",");
		//List<String> fList = Arrays.asList(fArray);
		List<String> fList = new ArrayList<String>();  
		for (String t : fArray) {
			fList.add(t.replaceAll("`", ""));
		}
		...
		for (int i = 0; i < links.length; i++) {
			...
			if(!fList.contains(link[0].replaceAll("`", ""))){
				try {
					fList.add(link[2]);
					finalQueryColumn += (","+HiveUtils.columsSpecialSymbols(link[0]));
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
		}
		return finalQueryColumn;
	}
	...
	public JSONObject hbaseProcessSerivce(JSONObject jsonParam) throws JSONException, ParseException {
		...
		if(tableName.equals("user_ucuser")){
...
//		}
		...
		try {
			HiveMetaService hiveMetaService = new HiveMetaService();
...
//				String[] typeValue = _columns.get(condition[0]);
				String typeValue = _columns.get(condition[0].replaceAll("`", "").trim());
				if(StringUtils.isNotBlank(typeValue)){
					String column = HiveUtils.columsSpecialSymbols(condition[0]);
					
					if(tableName.equals("event_logs") && column.equals("subeventcat")){
						if(condition[1].equals("0")){
							sb.append(" and subEventCat[\""+condition[2]+"\"] is not null ");
						}else if(condition[1].equals("3")){
							sb.append(" and subEventCat[\""+condition[2]+"\"] is null ");
						}else{
							continue;
						}
					}else{
						if(CommonUtil.isNumberType(typeValue) && !condition[1].equals("7")){
		...
		if(StringUtils.isNotBlank(filter)){
			...
			for(String _filter:filters){
				...
				try {
					...
					if(!"8".equals(condition[1])){
						comparatorCondition.put(condition[0], value+","+condition[1]);
					}else{
						String[] vs = value.split("___");
						if(vs!=null && vs.length==2){
							comparatorCondition.put(condition[0], vs[0]+",5,"+vs[1]+",2");
						}
					}
				} catch (JSONException e) {
					errorLogger.error(e.getMessage());
