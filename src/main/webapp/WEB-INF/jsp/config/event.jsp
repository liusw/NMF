...
<jsp:include page="/WEB-INF/jsp/common/top.jsp">
	...
	<jsp:param name="subnav" value="event" />
</jsp:include>

<div class="main">
	<jsp:include page="/WEB-INF/jsp/common/mainNav.jsp">
		<jsp:param name="actionName" value="事件管理" />
	</jsp:include>
			      ...
			      <div class="modal-body mf_modal_body">
			      	<div class="edit_area">
				      	<form class="form-horizontal" id="resForm">
				      	 	...
				      	 	<div class="control-field">
				      	 		<div class="field_body">
				      	 			<label class="field_label" for="inputEventCate">事件类型:</label>
				             		<div class="mf_field_input">
				             			<select id="inputEventCate" name="eventCate" class="selectpicker feild_value" data-live-search="true" style="display:none;" data-size="4">
				             			</select>
				             			<div class="msg-tip"></div>
				             		</div>
				      	 		</div>
...
$(document).ready(function() {
	...
	initTopPlats(function(){
		if(table){
			...
			$('#example tfoot th:eq(6)').html(selectStr);
			
			$('#example tfoot th:eq(6) select').on("change",function(){
				table.column(6).search(this.value).draw();
			});
			
			table.ajax.url("config/event/getData.htm?plat=" + $("#topPlats").val());
	...
	table = $('#example').DataTable({
		 "processing": true,
	     "sServerMethod": "POST",
	     "sAjaxSource" : "config/event/getData.htm?plat=" + $("#topPlats").val(),
	     "bServerSide" : true,
	     "bSort": false,
...
function _value() {
    if (table.$('tr.selected').get(0)) {
        ...
        $("#inputEventCate").val(selected.eventCateId);
...
function _addFun() {
    var jsonData = {
        ...
        'data[eventCateId]': $("#inputEventCate").val(),
    };
    $.ajax({
        url: "config/event/edit.htm",
        data: jsonData,
        type: "post",
        dataType:"json",
        success: function (backdata) {
            if (!backdata.error) {
                ...
                $("#myModal").modal("hide");
                resetFrom();
                table.ajax.url("config/event/getData.htm?plat=" + $("#topPlats").val()).load();
            } else{
...
function addSelectContent(){
	...
	var cateData = getJsonData("config/eventCate/getDataBySid.htm?plat=" + $("#inputPlat").val() + "&sid=" + $("#inputSite").val(),null);
	if(cateData.data!=null){
		$.each(cateData.data, function(index, value) {
			var label = value.ename + "(" + value.id + ")";
			$("#inputEventCate").append('<option value="' + value.id + '">' + label + '</option>');
		});
	}
	
	$("#inputEventCate").selectpicker('refresh');
	
	$("#inputPlat").on('change', function () {
		$("#inputSite").empty();
		
		$("#inputEventCate").empty();
