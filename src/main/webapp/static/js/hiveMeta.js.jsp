...
function editMeta(param){
	...
	param.tableName = $("#hiveMetaTableName").val();
...
function syncXml2Hive(){
	//var tableName = $("#hiveMetaTableName").val().toLowerCase();
	var tableName = $("#hiveMetaTableName").val();
	...
	$.ajax({
        type: "get",
        url:'hiveMeta/syncXml2Hive.htm?tableName='+tableName,
        ...
        success: function (data) {
        	 ...
        	 var table = $('#example').DataTable({
		         ...
		         "fnRowCallback":function(nRow,aData,iDataIndex){
		              	...
		              	var type = aData.type;
		              	var comment = aData.comment.replace(/'/g, "");
		              	var typeName = "字段";
		              	var column_name = aData.column_name;
		              	if(type == 1){
		              		typeName = "分区";
		              		}
		          		$('td:eq(-1)',nRow).html("<privilege:operate url='/#tmp#/updateHiveComment'><button type='button' class='btn btn-primary btn-xs' onclick='forEditMeta(" + aDataId + "," + type + ",\"" + column_name + "\"" + ",\"" + comment + "\")'>编辑</button></privilege:operate>");
...
function createHiveTabel(){
	...
	$.post("hiveMeta/createHivaTable.htm",{'hsql':hsql,'tableName': tableName},function(data){
